import { getPromptCommandId, getPromptCommandPrefix, stripGlobalPrefix } from '../../../common/constants';
...
export function createSetPromptKeybindingCommand(): Disposable {
  return registerCommand(Command.SetPromptKeybinding, async (item: TreePrompt) => {
    ...
    await openKeybindingsForCommand(getPromptCommandId(stripGlobalPrefix(item.promptName)));
  });
}
