...
export class ToolDocsGenerator {
  ...
  static getGlobalTools(): NonNullable<DevPanelConfig['tools']> {
    const config = ConfigManager.loadGlobalConfig();
    return config?.tools ?? [];
  }

  static generateToolsXml(workspaceFolder: WorkspaceFolder): string {
  ...
  static generateSkillMd(instructionsContent: string, toolName: string, toolCommand: string): string {
...
`;
  ...
  static async syncToSkills(workspaceFolder: WorkspaceFolder): Promise<number> {
    const config = ConfigManager.loadWorkspaceConfig(workspaceFolder);
    const localTools = config?.tools ?? [];
    ...
    const inactiveGlobalTools = globalTools.filter(
      (tool) => !activeTools.includes(`${GLOBAL_ITEM_PREFIX}${tool.name}`),
    );
  ...
  static syncToAiSpecs(xml: string, workspaceFolder: WorkspaceFolder) {
